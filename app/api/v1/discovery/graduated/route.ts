...
export const GET = withApiGateway(
  async (_ctx, request) => {
    try {
      ...
      const fingerprint = computeFilterFingerprint({ section: 'graduated', chain: params.chain, timeWindow: params.timeWindow });
      const offset = resolveOffset(params.offset, params.cursor, fingerprint);

      const allTokens = await getLiveDiscoveryTokens({ ...queryToFilter(params), section: 'graduated' });
