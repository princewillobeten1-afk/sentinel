...
export const GET = withApiGateway(
  async (_ctx, request) => {
    try {
      ...
      const fingerprint = computeFilterFingerprint({ section: 'movers', chain: params.chain, timeWindow: params.timeWindow });
      ...
      try {
        const cacheKey = `movers:${params.chain}:v24hChangePercent:desc::${params.limit}:${offset}`;
      ...
      return errorResponse(error instanceof Error ? error : new ApiError('Failed to fetch top movers', 500));
    }
  },
