...
export const GET = withApiGateway(
  async (_ctx, request) => {
    try {
      ...
      const allTokens = await getLiveDiscoveryTokens({ ...queryToFilter(params), section: 'volume' });
      ...
      return errorResponse(error instanceof Error ? error : new ApiError('Failed to fetch volume tokens', 500));
    }
  },
