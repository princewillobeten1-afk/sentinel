...
export const GET = withApiGateway(
  async (_ctx, request) => {
    try {
      ...
      try {
        const cacheKey = `liquidity:${params.chain}:liquidity:desc::${params.limit}:${offset}`;
      ...
      return errorResponse(error instanceof Error ? error : new ApiError('Failed to fetch liquidity discovery tokens', 500));
    }
  },
