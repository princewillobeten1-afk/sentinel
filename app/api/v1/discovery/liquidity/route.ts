...
export const GET = withApiGateway(
  async (_ctx, request) => {
    try {
      ...
      const fingerprint = computeFilterFingerprint({ section: 'liquidity', chain: params.chain, timeWindow: params.timeWindow });
      const offset = resolveOffset(params.offset, params.cursor, fingerprint);

      const allTokens = await getLiveDiscoveryTokens({ ...queryToFilter(params), section: 'liquidity' });
      ...
      return errorResponse(error instanceof Error ? error : new ApiError('Failed to fetch liquidity tokens', 500));
    }
  },
