/**
 * Creator Launch History API
 * GET /api/v1/creator/:chain/:address/history
 */
...
export const GET = withApiGateway(
  async (_ctx, _request, params) => {
    try {
      ...
      if (!creator) {
        throw new ApiError(`No creator history found for ${address}`, 404, 'NOT_FOUND');
      }

      return jsonResponse({
        creatorId: creator.creatorId,
        launches: creator.launches,
        count: creator.launches.length,
      });
    } catch (error) {
