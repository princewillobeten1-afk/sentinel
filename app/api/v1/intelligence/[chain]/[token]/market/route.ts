...
export const GET = withApiGateway(
  ...
  try {
    ...
    const marketDim = report.riskDimensions.MARKET;

    return jsonResponse({
      token: report.token,
      market: marketDim ? {
        score: marketDim.score,
        level: marketDim.level,
        confidence: marketDim.confidence,
        signals: marketDim.signals,
        evidence: marketDim.evidence,
        lastUpdated: marketDim.lastUpdated,
      } : null,
    });
  } catch (error) {
    return errorResponse(error instanceof Error ? error : new ApiError('Failed to fetch market data', 500));
  }
  },
