...
import { getWalletTransferList, getWalletTransferTotal } from '@/lib/api/birdeye/balance';
...
export async function GET(
  ...
  try {
    ...
    const tokenAddress = searchParams.get('token_address') || undefined;
    const flow = (searchParams.get('flow') as 'in' | 'out' | undefined) || undefined;
    ...
    const [transfers, totalResult] = await Promise.all([
      getWalletTransferList(address, {
        ...
        console.warn('Birdeye wallet transfer fallback:', err.message);
        return [];
      }),
      includeTotal
        ? getWalletTransferTotal(address, {
            token_address: tokenAddress,
            flow,
            time_from: timeFrom,
            time_to: timeTo,
    ...
    console.error('Wallet transfers route error:', error);
    return NextResponse.json(
      { success: false, error: error?.message || 'Failed to fetch wallet transfers' },
      { status: 500 }
    );
  }
}
