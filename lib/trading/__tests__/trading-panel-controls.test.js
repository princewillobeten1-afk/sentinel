...
describe('compact trading rail controls', () => {
  ...
  it('opens advanced conditions with the selected side', () => {
    mount(); fireEvent.click(screen.getByRole('button', { name: 'Sell', exact: true }));
    fireEvent.click(screen.getByLabelText('Advanced Trading Strategy'));
    fireEvent.click(screen.getByRole('button', { name: 'Add strategy' }));
    expect(screen.getByTestId('builder').textContent).toBe('advanced:sell');
  });
