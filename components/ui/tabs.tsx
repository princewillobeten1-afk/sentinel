...
export function Tabs({ tabs, activeTab, onChange, variant = 'segmented', size = 'md', className }: TabsProps) {
  if (variant === 'segmented') {
    return (
      <div className={clsx('inline-flex items-center rounded-lg bg-sentinel-950 p-1 border border-sentinel-800 select-none', className)}>
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isActive
                  ? 'bg-sentinel-750 text-slate-100 shadow-sm border border-sentinel-600/60 font-semibold'
  ...
  return (
    <div className={clsx('flex border-b border-sentinel-700/70 space-x-4 select-none', className)}>
      {tabs.map((tab) => {
        const isActive = tab.id === activeTab;
        return (
          <button
